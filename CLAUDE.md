...
- **Racers**: `state.racers` is an array; each racer has `{id, applicantType, r_rname, r_rsurname, r_idaddr, r_team, entries[], collapsed}`. `applicantType` is selected independently per racer as `person` or `company` for the existing demo account. Each entry is one race class `{cls, engine, carno}`. The data model still supports multiple racers and entries, but the current racer form intentionally does not render the add-racer or add-class buttons. `state.racer` is a flattened mirror of the first racer kept only so downstream screens keep working.
