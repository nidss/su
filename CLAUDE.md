...
- **Bank accounts**: `state.bank` stores both account groups. The applicant deposit-refund fields (`rf_*`) are rendered and saved with the personal-information form; the bank wizard screen contains only the winner prize-payment fields (`pz_*`). Both name/surname pairs are read-only in `bankAccountFields`: prize (`pz`) prefills from the first racer, while refund (`rf`) mirrors the applicant's name/surname live (`refundAccName`/`refundAccSurname`, kept in sync by `syncRefundName` in `bindApplicantForm`). Company applicants hide the refund surname.
...
- **Choice-before-onboarding flow**: The `choose` action-picker (racer vs VIP) is currently **hidden behind `const CHOOSE_ENABLED=false`** — its `SCREENS.choose`/`SCREEN_INIT.choose` code is intact but not shown. After register/login-new, `enterPostAuth()` skips straight into racer onboarding (`state.pendingChoice='racer'`), and every former back-target to `choose` routes through `chooseBackTarget()` (falls back to `auth` while hidden). Flip the flag to `true` to restore the picker. Choosing an action without personal info stores it in `state.pendingChoice` and opens `onboard`; `applicantFormInner({showRefund:false})` hides the applicant deposit-refund account for the VIP path, while racer onboarding keeps it required. If a VIP-only member later adds a racer, the profile routes them back through the prefilled onboarding form to collect that account first. The VIP option renders the original `suvip` UI in the isolated same-origin `vip.html` frame; `postMessage` prefills the shared member data, resizes the frame, records completed purchases in `state.profile.vips`, and returns the member button to the shared profile. Duplicate auth/profile/admin markup was removed from the embedded page.
