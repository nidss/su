...
- **`vip.html`** (`SCREENS.vip`, `vip.html?v=YYYYMMDDx`) — the buyer-facing VIP Village purchase flow. `index.html` prefills it and reads results back via `postMessage`; purchases land in `state.profile.vips`. Its own screen router toggles four views via `showView(v)`: `reg` → **`sum`** (payment summary: totals, payment terms, and the tax-invoice checkbox) → `pay` (payment method only) → `done`. Each ticket record carries `dayIds` and `plates` (`[{day,plate}]`).
- **`vip-dashboard.html`** (admin VIP tab, `vip-dashboard.html?v=YYYYMMDDx`) — a **complete second app** (its own `state`, i18n `L.th`/`L.en`, `el`/`esc`/`t`/`money`, and an admin dashboard). The VIP buyer table and its filter bar live here, **not** in `index.html`'s `DASH`. Registrations are read from the shared `suvip_regs` localStorage key. `renderDashboard()` builds the table from `filteredRegs()`; `seedMockIfEmpty()` seeds demo rows on first load. The first KPI card renders a **Chart.js doughnut** (`renderDayPie()`, tickets-per-event-day) using `chart.umd.min.js` — a **vendored** Chart.js v4.4.1 UMD build committed at repo root and loaded by a local `<script src>` (the org egress proxy blocks CDNs; keep it local). The buyer table has one column per event day (4/5/6 Dec) showing a check + license plate.

Each embedded file carries its own `?v=` suffix (they differ), so bump only the one you changed, in **both** `index.html` and `souped-up.html`. Editing `vip.html`/`vip-dashboard.html` needs no `souped-up.html` regeneration (only the `?v=` bump, applied in both).
...
- **Parking**: `pkSlots()` derives one booking slot per race-class entry across all racers; `state.parking.assign` maps `slotKey → pit`. `pkRows()` numbers pits continuously from 1–148: tents 12 and 13 contain two wide cells each (41–42 and 43–44) while retaining the width of four-cell tents. Booked spots show a text-only renter tooltip on hover/focus and retain the click modal. A selected `privatePit` is only an additional flag: it never disables the checkbox, clears assignments, or bypasses the requirement to select every pit. Cost is per car: `PRICES.entry + PRICES.deposit` × number of slots.
- **Account/Profile**: `state.account = {username, email, phone, password}`. Demo login uses three records in `DEMO_USERS`, keyed by `kind`: `new` starts onboarding with an empty profile (via `enterPostAuth()`); `existing` loads a populated profile with racers + history (its VIP tab is intentionally empty); `vip` is a buyer who never entered personal info (`buildVipOnlyProfile` → only `vips[]`). `state.profile` is built by `buildProfile` (existing), `buildVipOnlyProfile` (vip), or `buildProfileFromReg` (after payment). **`SCREENS.profile` renders two layouts** keyed on whether the profile has personal info (`p.applicant.a_name`/`a_company`, or any racers): with info → 4 tabs (`info` / `racers` / `vip` / `history`); without → no tabs, just the shared `profAccountCard()` (username/email/password) plus `profVipHTML()` (purchase history, each card showing `vipPlates(v)` — plates or `-`). `genVip()` seeds `dayIds` + `plates` so demo VIP cards and `vipReceiptPdfHTML` show a plate row.
...
- **Admin dashboard**: `Ctrl+Shift+Z` → admin login → shared Dashboard header with a real logout action, followed by a full-width registration/VIP switch. Registration keeps the KPI cards + paginated tables (`DASH` object, `renderDash`/`paintDashTable`). `buildDashData()` is **many-to-one**: each racer has an `applicantIdx` back-reference (one applicant can own several racers). The **applicant** tab (person/company) shows deposit-refund bank columns and an expand/collapse toggle whose detail row lists that applicant's racers (`dashRacerSubRow`, tent/pit included). The **racer** tab shows applicant name + tent + pit and omits ID-card address / vehicle-engine columns; tent/pit come from `pkPitTentPairs()` (derived from `pkRows()`). The VIP switch embeds `vip-dashboard.html`.
- **My Profile** (`SCREENS.profile`): reached by logging in or finishing a registration. Two layouts (see Account/Profile above). The racer-success screen (`SCREENS.done`) mirrors the full receipt on-screen — payer info + a per-car line-item table — reusing `pkSlots()`/`calcTotal()`.
