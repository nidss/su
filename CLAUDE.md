...
- **Existing demo account**: Login tags the account with `demoKind: "existing"`, loads a populated company applicant plus `buildExistingRefundBank()`, and shows the required `r_applicant_type` dropdown on the racer screen. New/demo-registered accounts do not see this racer applicant-type selector.
