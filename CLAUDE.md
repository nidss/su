...
- **Choice-before-onboarding flow**: New registrations and incomplete demo logins go to `choose` first. Choosing an action without personal info stores it in `state.pendingChoice` and opens `onboard`; after saving, `openChosenAction(target)` resumes the racer option at a fresh terms screen or opens the integrated VIP flow (`vip` → `vipPay` → `vipDone`). The VIP flow reuses `state.account`/`state.applicant` and stores completed purchases in `state.profile.vips`; it does not duplicate authentication, onboarding, profile, or admin screens.
